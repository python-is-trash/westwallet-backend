...
export const investmentService = {
  ...
  async getInvestments(telegramId) {
    ...
    const enriched = await Promise.all((investments || []).map(async (inv) => {
      ...
      return {
        ...
        can_claim: canClaim,
        is_matured: new Date() >= new Date(inv.end_time) && inv.investment_plans.duration_hours > 0,
        // Flatten plan fields for easier frontend access
        freeze_principal: inv.investment_plans.freeze_principal,
        claim_type: inv.investment_plans.claim_type,
        interest_rate_per_second: inv.investment_plans.interest_rate_per_second,
        duration_hours: inv.investment_plans.duration_hours,
      };
    }));
  ...
  async calculateCurrentProfit(investment) {
    ...
    if (plan.freeze_principal === false) {
      ...
      const profit = investment.amount * plan.interest_rate_per_second * elapsedSeconds;
      return Math.max(0, profit + (investment.accumulated_profit || 0));
    } else {
      if (now >= endTime) {
        return investment.return_amount - investment.amount;
      } else {
        const totalDuration = (endTime - startTime) / 1000;
        const elapsed = (now - startTime) / 1000;
        const profit = (investment.return_amount - investment.amount) * (elapsed / totalDuration);
  ...
  async claimProfit(telegramId, investmentId, claimType = 'profit') {
    ...
    // FLEXIBLE INVESTMENT (freeze_principal = false)
    if (plan.freeze_principal === false) {
      // Check cooldown for flexible investments
      ...
      if (claimType === 'principal_and_profit') {
        // Claim profit + principal and close investment
        claimedAmount = currentProfit + parseFloat(investment.amount);
        newStatus = 'completed';
      } else {
        // Claim only profit, keep investment running
    ...
    // LOCKED INVESTMENT (freeze_principal = true)
    else {
      // Check if locked investment has matured
      if (now < endTime) {
        const hoursRemaining = Math.ceil((endTime - now) / (1000 * 60 * 60));
        throw new Error(`Locked investment has not matured yet. ${hoursRemaining} hours remaining.`);
      }

      // For locked investments, only claim profit percentage
      // Principal returns automatically when timer ends
      claimedAmount = currentProfit;

      // If claiming after maturity, also return principal
      if (now >= endTime) {
        claimedAmount = currentProfit + parseFloat(investment.amount);
      }
